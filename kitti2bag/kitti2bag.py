...
def save_imu_data(writer, kitti, imu_frame_id, topic):
    ...
    for timestamp, oxts in zip(kitti.timestamps, kitti.oxts):
        ...
        imu.header.stamp = Time(
            seconds=float(timestamp.strftime("%s")),
            nanoseconds=int(float(timestamp.strftime("0.%f")) * 1e9),
        ).to_msg()
        ...
        writer.write(
            ...
            int(Time.from_msg(imu.header.stamp).nanoseconds),
        )
...
def save_dynamic_tf(writer, kitti, kitti_type, initial_time):
    ...
    if kitti_type.find("raw") != -1:
        for timestamp, oxts in zip(kitti.timestamps, kitti.oxts):
            ...
            tf_oxts_transform.header.stamp = Time(
                seconds=float(timestamp.strftime("%s")),
                nanoseconds=int(float(timestamp.strftime("0.%f")) * 1e9),
            ).to_msg()
...
def save_camera_data(
    ...
    if kitti_type.find("raw") != -1:
        ...
        image_filenames = sorted(os.listdir(image_path))
        image_datetimes = map(
            lambda x: initial_time + x.total_seconds(), kitti.timestamps
        )
    ...
    for dt, filename in bar(iterable):
        ...
        if kitti_type.find("raw") != -1:
            image_message.header.stamp = Time(
                seconds=float(datetime.strftime(dt, "%s")),
                nanoseconds=int(float(datetime.strftime(dt, "0.%f")) * 1e9),
            ).to_msg()
            topic_ext = "/image_raw"
        elif kitti_type.find("odom") != -1:
...
def run_kitti2bag():
    ...
    if args.kitti_type.find("raw") != -1:
        ...
        bag_filename = "kitti_{}_drive_{}_{}_ros2".format(
            args.date, args.drive, args.kitti_type[4:]
        )

        # Create bag writer
        storage_options = rosbag2_py.StorageOptions(
            uri=bag_filename, storage_id="sqlite3"
        )
        ...
        bag_filename = "kitti_data_odometry_{}_sequence_{}_ros2".format(
